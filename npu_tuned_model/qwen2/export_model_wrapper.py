...
class Qwen2ForCausalLMWrapper(nn.Module):
    def __init__(self, model_path, device, dtype, embedding_config):
        super(Qwen2ForCausalLMWrapper, self).__init__()
        self.model = Qwen2ForCausalLM.from_pretrained(model_path, device_map=device, torch_dtype=dtype, trust_remote_code=False)
        self.model.eval()
