...
class GlmRotaryEmbedding(nn.Module):
    ...
    def _precompute_sin_cos(self):
        # 创建一些示例的position_ids用于计算sin和cos的缓存值
        # 这里假设position_ids的形状是[1, max_position_embeddings]
        batch = 1
        seq_len = 1
        position_ids_shape = [batch, seq_len]
        position_ids = torch.ones(position_ids_shape, dtype=torch.int64).to(self.inv_freq.device)
        # position_ids = torch.arange(0, self.max_seq_len_cached).unsqueeze(0).to(self.inv_freq.device)
        inv_freq_expanded = self.inv_freq[None, :, None].float().expand(position_ids.shape[0], -1, 1)
        position_ids_expanded = position_ids[:, None, :].float()

        device_type = self.inv_freq.device.type
...
def rotate_half(x):
    ...
    #return torch.stack((-1 * x2, x1), dim=-1).flatten(-2)
...
class GlmAttention(nn.Module):
    ...
    def __init__(self, config: GlmConfig, layer_idx: Optional[int] = None):
        ...
        self.q_proj = FLinearMatmul(nn.Linear(self.hidden_size, self.num_heads * self.head_dim, bias=config.attention_bias))
        self.k_proj = FLinearMatmul(nn.Linear(self.hidden_size, self.num_key_value_heads * self.head_dim, bias=config.attention_bias))
        self.v_proj = FLinearMatmul(nn.Linear(self.hidden_size, self.num_key_value_heads * self.head_dim, bias=config.attention_bias))
        self.o_proj = FLinearMatmul(nn.Linear(self.hidden_size, self.hidden_size, bias=False))
    ...
    def forward(
        ...
        **kwargs,
        ...
        cos, sin = position_embeddings # 这里需要注意position_embeddings = self.rotary_emb(hidden_states, position_ids) 在model中计算，如果导出单算子，不过经过model计算，因此position_embedding会为空
        query_states, key_states = apply_rotary_pos_emb(query_states, key_states, cos, sin)
        kv_cache_max_len = self.config.kv_cache_max_len
        if kv_cache_max_len is not None:
            kv_seq_len = kv_cache_max_len
        if kv_cache_max_len is not None and new_kv_cache_pos is not None:
        ...
        if kv_cache_max_len is not None and new_kv_cache_pos is not None:
            key_states = repeat_kv(key_states.transpose(0, 2), self.num_key_value_groups)
            value_states = repeat_kv(value_states.transpose(0, 2), self.num_key_value_groups)
        else:
