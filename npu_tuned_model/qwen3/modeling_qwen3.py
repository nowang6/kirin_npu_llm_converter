...
class Qwen3Attention(nn.Module):
    ...
    def forward(
        ...
        query_states = self.q_norm(self.q_proj(hidden_states).view(bsz, -1, self.num_heads, self.head_dim)).transpose(1, 2)
        key_states = self.k_norm(self.k_proj(hidden_states).view(bsz, -1, self.num_key_value_heads, self.head_dim)).transpose(1, 2)
        ...
        # import pdb
        # pdb.set_trace()
        last_dim_tmp = query_states.shape[1] * query_states.shape[3]
        attn_output = attn_output.reshape(*input_shape, -1).contiguous().reshape(-1, last_dim_tmp)
        # attn_output = attn_output.reshape(-1, self.hidden_size)
        attn_output = self.o_proj(attn_output).view(bsz, -1, self.hidden_size)
